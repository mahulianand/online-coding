...
/*
 The Minimum Number Of Moves Problem Code: SALARY
 
All submissions for this problem are available.
Little chief has his own restaurant in the city. There are N workers there. Each worker has his own salary. 
The salary of the i-th worker equals to Wi (i = 1, 2, ..., N). Once, chief decided to equalize all workers, that is, he wants to make salaries of all workers to be equal. 
...
public class SALARY {
	...
	public static void main(String[] args) {
		...
		for (int testNo = 0; testNo < numberOfTestCases; testNo++) {
			int steps = 0;
			...
			Arrays.parallelSort(salaries);

			int min = salaries[0];
			int max = salaries[salaries.length - 1];
			steps = max - min;
			for (int j = 0; j < salaries.length - 2; j++) {
				if ( salaries[j + 1] != salaries[j]) {
					steps+=(salaries[j + 1] - salaries[j]);
				}
			}
